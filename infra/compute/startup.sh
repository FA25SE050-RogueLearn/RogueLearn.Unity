#!/bin/bash
# Compute Engine startup script for Unity headless server container
# - Installs Docker and Cloud Ops Agent
# - Reads runtime configuration from instance metadata
# - Runs the container exposing HTTP health on 8080 and persisting logs under /var/log/unity
...
# Read configuration from metadata (provide these via --metadata when creating the VM)
IMAGE="$(get_md image)"
...
# Install Cloud Ops Agent (optional; can help ship logs/metrics)
...
# If a previous container exists, stop/remove it to ensure a clean start
if docker ps -a --format '{{.Names}}' | grep -q '^unity-server$'; then
  docker rm -f unity-server || true
fi

# Run the container
# -p 8080:8080 exposes health endpoint
# -v /var/log/unity:/var/log/unity persists logs and match results JSON files
# Env vars configure scene, Relay region, max connections, and results sinks
docker run --name unity-server --restart=always \
  -p 8080:8080 \
  -e UNITY_SERVER_SCENE="${UNITY_SERVER_SCENE}" \
  -e RELAY_REGION="${RELAY_REGION}" \
  -e RL_MAX_CONNECTIONS="${RL_MAX_CONNECTIONS}" \
  -e RESULTS_SINK="${RESULTS_SINK}" \
  -e RESULTS_LOG_ROOT="${RESULTS_LOG_ROOT}" \
  -e RESULTS_HTTP_URL="${RESULTS_HTTP_URL}" \
  -e RESULTS_HTTP_TOKEN="${RESULTS_HTTP_TOKEN}" \
  -v /var/log/unity:/var/log/unity \
  "${IMAGE}"

echo "[startup] Container launched: unity-server"
echo "[startup] Health probe: http://<VM_EXTERNAL_IP>:8080/health"
